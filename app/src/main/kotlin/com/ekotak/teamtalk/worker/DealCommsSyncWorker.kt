...
/**
 * Wysyła to, co napisano na zakładce „Komunikacja" bez zasięgu: wpisy do
 * zespołu, wiadomości WhatsApp i streszczenia rozmów.
 *
 * Zamawia go repozytorium przy każdym zakolejkowanym wpisie, z warunkiem sieci —
 ...
 * proces zginął z pełną kolejką: rozmowa u klienta bywa długa, a telefon
 ...
 */
@HiltWorker
class DealCommsSyncWorker @AssistedInject constructor(
    ...
    companion object {
        const val UNIQUE_NAME = "deal_comms_sync"
    }

    override suspend fun doWork(): Result {
        sessionPreferences.token.first() ?: return Result.success()

        return when (runCatching { comms.syncPendingMutations() }.getOrNull()) {
